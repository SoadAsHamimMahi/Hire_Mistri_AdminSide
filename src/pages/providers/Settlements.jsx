...
export default function Settlements() {
  ...
  const [createData, setCreateData] = useState({ providerId: '', amount: '' })
  ...
  const handleStatusChange = async (id, newStatus) => {
    if (!window.confirm(`Are you sure you want to mark this settlement as ${newStatus}?`)) return
    setActionLoadingId(id)
    try {
      await api.patch(`/api/admin/settlements/${id}`, { status: newStatus })
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      setCreateData({ providerId: '', amount: '' })
      fetchSettlements()
    } catch (err) {
  ...
  const statusBadge = (status) => {
    ...
    if (s === 'completed') return <span className="badge badge-success badge-sm">Completed</span>
    if (s === 'failed') return <span className="badge badge-error badge-sm">Failed</span>
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="mt-1 text-slate-600">Track and manage payouts to providers.</p>
        </div>
        <button className="btn btn-primary" onClick={() => setShowCreateModal(true)}>
          <PlusIcon className="w-5 h-5" /> Create Settlement
        </button>
      </div>
